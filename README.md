...
## diary of progress:
* 20160626 __processing__ a bunch of files need to be figured out since this type of computing will be common.  I think something like applyFuncToListOfFiles() would be good, but i need to figure out how to systematically save the outputs as well.  Perhaps the function you pass has instructions on how to save its own output?  no, this makes it not so general purpose.  how bout you pass the saving function as well? so then the user can select a generic saving function that works most of the time, and if need be make a custom saving function for special cases.  yup.  this is how i will do it: applyFuncTo_ListOfFiles(ListOfFiles,@myFunc,@mySavingDirections,{parameterListForMyFunc},{paramterListForSavingDirections})

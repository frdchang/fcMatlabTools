function [h,conditionHolder] = analyzeStageIFPR(benchStruct,conditionFunc,field,varargin)
% first plots cdf
...
setupParForProgress(prod(sizeAB));
myFPR = zeros(sizeAB(2),1);
Baxis = zeros(size(myFPR));
% define threshold at first index when b = 0 (this better be true)
thresh = 0.99;
ii = 1;
sig = conditionHolder{ii}.sig;
bk = conditionHolder{ii}.bk;
ROC = genROC('adsf',sig,bk,'doPlot',false);
useIDX = find(ROC.withoutTargetCDF > thresh,1);
useThresh = ROC.newDomain(useIDX);
myFPR(ii) = 1-ROC.withoutTargetCDF(useIDX);
Baxis(1) = conditions{1}.B;
for ii = 2:sizeAB(2)
    %     display(ii);
    incrementParForProgress();
    sig = conditionHolder{ii}.sig;
